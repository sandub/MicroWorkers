...
class PS_Pagination {
	...
	function paginate() {
		...
		$all_rs = mysql_query($this->sql)or die(mysql_error());
		if(!$all_rs) {
	 ...
	 * @return string
	 */
	function renderFirst($param,$tag='First') {
		if($this->page == 1) {
			return $tag;
		}
		else {
			return '<a href="'.$this->php_self.'?'.$param.'page=1">'.$tag.'</a>';
		}
	}
	 ...
	 * @return string
	 */
	function renderLast($param,$tag='Last') {
		if($this->page == $this->max_pages) {
			return $tag;
		}
		else {
			return '<a href="'.$this->php_self.'?'.$param.'page='.$this->max_pages.'">'.$tag.'</a>';
		}
	}
	 ...
	 * @return string
	 */
	function renderNext($param,$tag='Next') {
		if($this->page < $this->max_pages) {
			return '<a href="'.$this->php_self.'?'.$param.'page='.($this->page+1).'">'.$tag.'</a>';
		}
		else {
			return $tag;
		}
	 ...
	 * @return string
	 */
	function renderPrev($param,$tag='Prev') {
		if($this->page > 1) {
			return '<a href="'.$this->php_self.'?'.$param.'page='.($this->page-1).'">'.$tag.'</a>';
		}
		else {
	 ...
	 * @return string
	 */
	function renderFullNav($param) {
	 $this->total_rows;
	 if($this->total_rows > $this->rows_per_page)
	 {
		return $this->renderFirst($param).'&nbsp;'.$this->renderPrev($param).'&nbsp;'.$this->renderNav($param).'&nbsp;'.$this->renderNext($param).'&nbsp;'.$this->renderLast($param);	
	}
	else
	{
		return '';
