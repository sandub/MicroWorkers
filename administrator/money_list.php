...
if(isset($_GET["order"]) && $_GET["order"]!='') {
	$sql = "select * from `jobs`,`user_registration` where `user_registration`.`id`=`jobs`.`user_id` and `jobs`.`email`<>'support@mymicrojob.com' order by `user_registration`.`".$_GET["order"]."`";
} else {
	$sql = "select * from `jobs` where `email`<>'support@mymicrojob.com' order by `title`";
}
...
<form name="searching" id="searching" action="" method="post">
    <tr>
...
<form name="ff" id="ff" action="managejobs.php" method="post" onSubmit="return godelete();">
    ...
    <tr>     
		...
		<td width="189" align="left" valign="top" class="labels1"><strong>Job Name </strong></td>    
        <td width="46" align="left" valign="top" class="labels1"><strong>Job ID</strong></td> 		
        <td width="215" align="left" valign="top" class="labels1"><strong><a href="money_list.php?order=fullname">User Name</a></strong></td>
		<td width="83" align="center" valign="top" class="labels1"><strong>Fees</strong></td> 	
		<td width="145" align="center" valign="top" class="labels1"><strong><?php echo $FeePerCompletedJob; ?>% Fee On<br />(No Job x Paid)</strong></td>
		<td width="190" align="center" valign="top" class="labels1"><strong>Started</strong></td>	
    </tr>
    <?php
	$ul=0;	
	...
	$count=1;
		...
		$user=dbFetchArray(dbQuery("select * from `user_registration` where `id`='".$t["user_id"]."'"),MYSQL_BOTH);
		$fullname=stripslashes($user['fullname']);
		
		$admin_earn=$admin_earn+$t["extra_price"];
		$extra=$admin_earn;
		
		$worker=$worker+($t["price"]*$t["wd1"]);
		$worker_get=$worker;
	?>	
	...
	<td align="center" valign="top" class="<?php if($ul%2==0) {?>rows<?php } else {?>labels<?php } ?>"><?php echo $t["wd1"].' x $'.$t["price"].' = $'.number_format($t["wd1"]*$t["price"], 2,'.',''); ?></td>    
	...
	$ul=$ul+1;
	  ...
	  <tr id="tr<?php echo $ul; ?>">
		...
		<span style="font-weight:bold; font-size:12px;">Admin Will Earn : <?=number_format($extra, 2);?> + (5% of $<?=number_format($worker_get, 2);?>) $<?=number_format($mine, 2);?> = $<?=number_format($total, 2);?></span></td>
	    <td align="left" valign="top" class="<?php if($ul%2==0) {?>rows<?php } else {?>labels<?php } ?>">&nbsp;</td>
      </tr>
	<?
	}
	...
	<?php if($count==1) { ?>                         
    <tr>
    	<td valign="top" colspan="7" class="labels"><div style=" width: 734px; float:left;"><div style="width:70px; float:left; text-align:left;"><button class="input" style="font-size:9px" type="submit" name="delete">DELETE</button></div><div style="width:654px; float:left; text-align:center;"><?php echo $pagelist; ?></div></div></td>
	</tr>
	<?php } ?>
</form>
</table>
